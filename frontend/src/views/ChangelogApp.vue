...
const changelogEntries = [
  {
    ...
    title: '方差分析的精度跃迁',
    items: [
      '事后多重比较迎来精度跃迁：SNK / Duncan 的 rank_distance 之前一直是空值裸跑，永远退化成了 k=2 的最宽松口径，你现在去翻之前的输出，P 值全是错的。修好之后按均值间距逐步收紧，该宽的地方宽该严的地方严，这才是真正的多重比较。',
      '方差分析不再有天花板：从今天起，两因素、三因素写成过去式了。N 因素方差分析正式上线，你有几个自变量就丢几个，交互项自动配，R 引擎底层兜底，ANOVA 的表现力从此没有上限。',
      '打穿 Python 和 R 的墙：之前加一个 R 分析要对着源码猜参数格式，现在一份接口对齐文档把协议讲透了，Python 调 R 不再是黑魔法，新人上手直接读文档就行。',
      '补了全套回归测试：新功能从参数传入到结果输出全线自动化验证，以后改代码不用凭手感，跑一遍测试就知道炸没炸。',
    ],
  },
  {
    date: '2026-05-18',
