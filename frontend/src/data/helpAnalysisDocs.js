...
export const analysisChildren = [
  ...
  buildAnalysisChild('analysis-data-overview', '数据探查', {
    intro: '数据探查用于全面检查当前数据集的规模、变量类型、缺失与异常，评估数据可用度，是正式分析前最适合先看的入口。',
    purpose: [
    ...
    steps: [
      {
        title: '选择"数据探查"方法',
        text: '在常用方法分类下点击"数据探查"，进入当前数据集的探查界面。',
      },
      {
        title: '拖入需要查看的变量',
    ...
    formulas: [
      {
        title: '缺失比例',
        text: '数据探查里最常见的一个指标是每个变量的缺失比例，用来判断后续是否需要清洗。',
        latex: String.raw`$$
...
summary["non_missing"] = len(df) - summary["missing_count"]`,
  ...
  buildAnalysisChild('analysis-frequency', '频数分析', {
    ...
    steps: [
      {
        title: '选择“频数分析”',
        text: '在常用方法分类下点击"频数分析"，进入单变量分布统计页面。',
      },
      {
        title: '拖入分析变量',
...
result = pd.DataFrame({
  ...
  buildAnalysisChild('analysis-descriptive', '描述性统计', {
    ...
    steps: [
      {
        title: '选择“描述性统计”',
        text: '在常用方法分类下点击“描述性统计”，进入多变量描述统计界面。',
      },
      {
        title: '拖入定量变量',
...
desc = desc.rename(columns={
  ...
  buildAnalysisChild('analysis-category-summary', '分类汇总', {
    intro: '分类汇总用于按一个分类变量分组，汇总一个或多个定量变量的所选统计量，适合先做分组概览再决定是否进入差异检验。',
    io: [
    ...
    steps: [
      {
        title: '选择“分类汇总”',
        text: '在常用方法分类下点击“分类汇总”，准备按分组变量查看不同类别下的基本统计情况。',
      },
      {
        title: '拖入分类变量和汇总变量',
...
$$`,
  ...
  buildAnalysisChild('analysis-cross-tabulation', '卡方（交叉）分析', {
    intro: '卡方（交叉）分析用于同时查看两个分类变量的交叉分布，并结合卡方检验与关联强度评价类别之间的关系。',
    io: [
    ...
    steps: [
      {
        title: '选择“卡方（交叉）分析”',
        text: '在常用方法分类下点击该方法，准备查看两个分类变量之间的分布关系。',
      },
      {
        title: '分别拖入行变量和列变量',
...
cramers_v = np.sqrt(chi2 / (table.to_numpy().sum() * min(table.shape[0] - 1, table.shape[1] - 1)))`,
  ...
  buildAnalysisChild('analysis-normality', '正态性分析', {
    ...
    steps: [
      {
        title: '选择“正态性分析”',
        text: '在常用方法分类下点击“正态性分析”，进入正态性检验页面。',
      },
      {
        title: '拖入定量变量',
...
chi2, p, dof, expected = chi2_contingency(table)`,
  ...
  buildAnalysisChild('analysis-correspondence', '对应分析', {
    intro: '对应分析用于把多个定类变量的类别结构映射到低维空间中，帮助你观察哪些类别彼此更接近、哪些类别更远离。',
    io: [
      '输入：2 个及以上定类变量，通常来自问卷偏好题、品牌认知题、人群细分题等。',
    ...
    steps: [
      ...
      {
        title: '拖入定类变量',
        text: '优先选择类别数量适中、且业务上有解释意义的变量，变量数至少为2，避免类别过多导致图形难以阅读。',
      },
      {
        title: '执行并查看类别坐标',
...
print(model.params)`,
  ...
  buildAnalysisChild('analysis-vif', '共线性分析', {
    intro: '共线性分析用于判断多个自变量之间是否过于相似或重叠，是回归建模前后都非常值得检查的指标。',
    io: [
      '输入：2 个或以上用于建模的定量自变量。',
