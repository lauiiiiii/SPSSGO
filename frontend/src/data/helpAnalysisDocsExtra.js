...
const extendedStatChildren = [
  ...
  buildAnalysisChild('analysis-gof-chi', '卡方拟合优度检验', {
    ...
    io: ['输入：1 个分类变量，可按类别填写期望比例；不填时默认等比例。', '输出：实际频数、期望频数、残差、χ²、P值和期望频数图。'],
  }),
  ...
  buildAnalysisChild('analysis-kappa', 'Kappa一致性检验', {
    intro: 'Kappa 一致性检验用于评估两个及以上评价者或分类结果之间的一致性，支持简单Kappa、线性加权Kappa、平方加权Kappa和Fleiss Kappa。简单/加权 Kappa 主表标准误走 Fleiss-Cohen-Everitt 1969 大样本渐近方差：标准误(假定原假设) 用于检验 Kappa=0、标准误 用于 95% CI，两者对应两个不同方差公式；Fleiss Kappa 主表标准误走 Fleiss-Levin-Paik 2003 H0=0 下大样本渐近方差；po、pe、Kappa 按 Cohen 1960 / Fleiss 1971 原始公式手算。',
    io: ['输入：2 个及以上定量或定类变量，可选 1 个权重变量；权重列若是字符串型 ID（如"学生1..学生50"）会自动抽出数字部分作为频数权重，避免主分析退化成未加权。', '输出：po、pe、Kappa值、标准误(假定原假设)、标准误、z值、P值、95%CI、评价者交叉表（方便和外部参考软件的列联表逐格对照）和热力图；简单 Kappa 详细结论里附 Cohen 1960 经典手算近似 SE=sqrt(po(1-po)/(N(1-pe))) 作为另一种量级参考。'],
  }),
  buildAnalysisChild('analysis-kendall-consistency', 'Kendall一致性检验', {
    intro: 'Kendall 一致性检验用于评估多位评价者或专家对同一批评价对象排序结果的一致性程度。',
    io: ['输入：2 个及以上被评价对象/指标变量，每行代表一位评价者或专家的评分。', '输出：各对象的秩平均值、中位数、Kendall’s W、卡方值和显著性结果；并列分数按平均秩处理并做并列修正。'],
  }),
  buildAnalysisChild('analysis-icc', 'ICC组内相关系数', {
    intro: 'ICC组内相关系数用于评估多个评价者或重复测量之间的一致性和可靠性，可在双向混合/随机绝对一致性、双向混合/随机一致性和单向随机绝对一致性之间选择。',
    io: ['输入：2 个及以上评价变量；ID列不会自动剔除，拖入后会按普通定量列参与计算。', '输出：所选 ICC 类型下的单一度量和平均度量、95%置信区间、F检验自由度和显著性P值。'],
  }),
  buildAnalysisChild('analysis-correlation-auto-solver', '相关与一致性推荐', {
    intro: '相关与一致性推荐用于在你不确定该用 Pearson、Spearman、Kappa、ICC 还是 Cochran’s Q 时，自动识别变量特征并给出推荐方法。',
    io: ['输入：2 个或多个待分析变量，可混合连续数值、等级变量、分类变量或二分类变量。', '输出：变量识别结果、推荐方法列表，以及首选方法的推荐结果。'],
    steps: [
      { title: '选择“相关与一致性推荐”', text: '先进入该方法页，把你暂时不确定该如何处理的变量一起放进来。' },
      { title: '拖入待判断变量', text: '可以一次放入多个变量，系统会自动识别是连续数值、等级变量、分类变量还是二分类变量。' },
      { title: '执行并查看推荐方法', text: '先看变量识别结果，再看推荐方法列表，最后阅读推荐出的首选结果。' },
    ],
    reading: [
      { title: '先看识别类型', text: '相关与一致性推荐会先判断每个变量更像连续变量、等级变量还是分类变量，这一步决定后续推荐路径。' },
    ...
    notice: '相关与一致性推荐更适合作为“方法推荐入口”。如果你的研究目标已经非常明确，例如就是要做一致性评价或二分类重复测量检验，建议直接进入对应方法。',
  }),
