import { _ as _export_sfc, o as openBlock, c as createElementBlock, j as createBaseVNode, a as createTextVNode } from "./chunks/framework.BsdvCmHD.js";
const __pageData = JSON.parse('{"title":"配对样本T检验","description":"","frontmatter":{},"headers":[],"relativePath":"analysis/paired-t.md","filePath":"analysis/paired-t.md"}');
const _sfc_main = { name: "analysis/paired-t.md" };
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return openBlock(), createElementBlock("div", null, [..._cache[0] || (_cache[0] = [
    createBaseVNode("h1", {
      id: "配对样本t检验",
      tabindex: "-1"
    }, [
      createTextVNode("配对样本T检验 "),
      createBaseVNode("a", {
        class: "header-anchor",
        href: "#配对样本t检验",
        "aria-label": 'Permalink to "配对样本T检验"'
      }, "​")
    ], -1),
    createBaseVNode("p", null, "analysis-paired-t", -1),
    createBaseVNode("h2", {
      id: "输入输出描述",
      tabindex: "-1"
    }, [
      createTextVNode("输入输出描述 "),
      createBaseVNode("a", {
        class: "header-anchor",
        href: "#输入输出描述",
        "aria-label": 'Permalink to "输入输出描述"'
      }, "​")
    ], -1),
    createBaseVNode("ul", null, [
      createBaseVNode("li", null, "输入：2 个一一对应的定量变量，例如前测与后测、干预前与干预后、A 条件与 B 条件。"),
      createBaseVNode("li", null, "输出：配对描述统计、均值差、t 检验结果和效应量。")
    ], -1)
  ])]);
}
const pairedT = /* @__PURE__ */ _export_sfc(_sfc_main, [["render", _sfc_render]]);
export {
  __pageData,
  pairedT as default
};
