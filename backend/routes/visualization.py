...
from backend.services.visualization_service import save_visualization_result
...
@router.post("/api/visualizations/{session_id}/save")
async def visualization_save(session_id: str, req: VisualizationSaveRequest, user=Depends(current_user_required)):
    await get_session_or_404(session_id, user)
    payload = req.model_dump() if hasattr(req, "model_dump") else req.dict()
    return await save_visualization_result(session_id, payload)
