...
def run(df, params):
    ...
    for group in groups:
        ...
        for variable in multiple_vars:
            rate = _selected_mask(temp.loc[group_mask, variable]).mean() * 100 if base else 0
            row.append(f"{_fmt(rate, 1)}%")
        rows.append(row)

    sections = [
        _sec_table("单选结果下的多选偏好", headers, rows, description="表格展示不同单选结果对应人群在各多选项上的选择比例。"),
        _sec_advice("该方法适合从单选结果出发看偏好扩展。若需要显著性判断，可继续在当前模块中加入比例差异或卡方检验。"),
        _sec_smart(f"本次围绕单选变量 {single_var} 展开分析，共比较了 {len(groups)} 个单选结果对应的人群偏好。"),
