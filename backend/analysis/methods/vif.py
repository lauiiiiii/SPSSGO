...
METHOD_KEY = "vif"
METHOD_META = {'label': '多重共线性 VIF',
 'category': '回归&因果分析包',
 'description': '检测多个自变量之间是否存在多重共线性',
 'slots': [{'key': 'variables',
            'label': '分析变量',
            'type': 'multiple',
            'accept': 'numeric',
...
def vif_analysis(df, params):
    ...
    if len(variables) < 2:
        return {"name": "共线性诊断", "headers": [], "rows": [], "description": "需要至少2个变量。"}

    temp = df[variables].apply(pd.to_numeric, errors="coerce").dropna()
    if temp.empty:
        return {"name": "共线性诊断", "headers": [], "rows": [], "description": "有效样本不足。" }
    ...
    sections = []
    sections.append(_sec_table("共线性诊断结果", headers, rows,
                               description="VIF（方差膨胀因子）用于检测自变量之间的多重共线性。容忍度=1/VIF。"))

    advice = (
        "共线性诊断用于检测多个自变量之间是否存在多重共线性；\n"
        "第一：VIF<5，不存在共线性问题；\n"
    ...
    return {"name": "共线性诊断（VIF）", "headers": headers, "rows": rows, "description": smart, "sections": sections}
